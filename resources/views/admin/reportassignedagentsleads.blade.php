...
@push('head')
    <title>Assigned Agents For Leads</title>
...
@section('wrapper_content')
    ...
    <div class="container-fluid" style="padding-left:0!important">
        <div class="row">
            <div class="col-lg-4 col-md-5 col-8">
                <h3 class="top-title">Assigned Agents For Leads</h3>
            </div>
        </div>
    </div>
    <div id="content-wrapper" class="d-flex flex-column">
        <div class="row mb-5 mt-5 d-flex justify-content-center">
            {{-- <div class="col-md-4">
                <label for="">Data Soruce</label>
                <select class="form-control" name="source" id="data_soruce">
                    @foreach ($soruces as $soruce)
                        <option value="{{ $soruce->source }}">{{ $soruce->source }}</option>
                    @endforeach
                </select>
            </div> --}}
        </div>
            ...
            <table class="table table-bordered table-hover" style="width: 200%" id="dataTable">
                <thead style="background: #70cacc;color: aliceblue;">
                    <tr>
                        ...
                        <th><span class="table-title">Agent</span></th>
                    </tr>
                </thead>
                <tbody>
                </tbody>
...
@push('scripts')
    ...
    <script>
        let table = $('#dataTable').DataTable({
            ...
            "ajax": {
                "url": "{{ route('reportassignedagentsleads') }}",
                "dataType": "json",
                "type": "POST",
                "data": function(d) {
                    d._token= "{{ csrf_token() }}";
                    d.source= $('#data_soruce').val();
                },
                beforeSend: function() {
