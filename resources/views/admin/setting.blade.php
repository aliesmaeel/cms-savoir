    ...
    <div class="container-fluid" style="padding-left:0!important">
        <div class="row">
            <div class="col-lg-4 col-md-5 col-8">
                <h3 class="top-title">Logo Setting</h3>
            </div>
        </div>
    </div>
    <div id="content-wrapper" class="d-flex flex-column">
        <span id="field_errors" class="mt-4"></span>
        <form id="maindata" action="{{ route('setting.logo') }}" method="POST">
            <div class="row mt-4">
                <div class="col-md-12">
                    <label for="watermark">Select Logo</label>
                    <input class="form-control" type="file" name="watermark" id="watermark" onchange="readURL(this);">
                    <div id="img1">
                        @if ($data['logo'])
                            <img src="{{ config('services.cms_link').'/'.$data['logo'] }}" alt="" width="200" height="200"
                                id="logo_img">
                        @endif
                    </div>
                </div>
            ...
            <div class="row mt-5 mb-5">
                <div class="col-md-4">
                    ...
                    <input class="form-control" type="number" min="0" name="watermark_width" id="watermark_width"
                        value="{{ $data['watermark_width'] }}" required>
                </div>
                <div class="col-md-4">
                    <label for="watermark_height">Height</label>
                    <input class="form-control" type="number" min="0" name="watermark_height" id="watermark_height"
                        value="{{ $data['watermark_height'] }}" required>
                </div>
                <div class="col-md-4">
                    <label for="watermark_position">Position</label>
                    <select name="watermark_position" id="watermark_position" class="form-control" required>
                        <option {{ $data['watermark_position'] == 'top-left' ? 'selected' : '' }} value="top-left">top-left
                        </option>
                        <option {{ $data['watermark_position'] == 'top' ? 'selected' : '' }} value="top">top</option>
                        <option {{ $data['watermark_position'] == 'top-right' ? 'selected' : '' }} value="top-right">
                            top-right</option>
                        <option {{ $data['watermark_position'] == 'left' ? 'selected' : '' }} value="left">left</option>
                        <option {{ $data['watermark_position'] == 'center' ? 'selected' : '' }} value="center">center
                        </option>
                        <option {{ $data['watermark_position'] == 'right' ? 'selected' : '' }} value="right">right
                        </option>
                        <option {{ $data['watermark_position'] == 'bottom-left' ? 'selected' : '' }} value="bottom-left">
                            bottom-left</option>
                        <option {{ $data['watermark_position'] == 'bottom' ? 'selected' : '' }} value="bottom">bottom
                        </option>
                        <option {{ $data['watermark_position'] == 'bottom-right' ? 'selected' : '' }} value="bottom-right">
                            bottom-right</option>
                    </select>
                </div>
            </div>
            <div class="row mt-5">
                <div class="col-md-12 text-center">
                    <button id="buttonsubmit" type="submit"
                        class="registerbtn">{{ $data['watermark_height'] ? 'Update' : 'Add' }}
...
@push('scripts')
    ...
    <script>
        $('#maindata').submit(function(e) {
            ...
            formData.append('file', $('#watermark')[0].files[0]);
            $.ajax({
        ...
        function readURL(input) {
            if (input.files && input.files[0]) {
                var reader = new FileReader();
                reader.onload = function(e) {
                    $('#logo_img').attr('src', e.target.result).width(200).height(200);
                };
