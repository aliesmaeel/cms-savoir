    ...
    <style>
        ...
        * {
        ...
        #datetimepicker3 {
            ...
            text-align: left;
            border-right: 3px solid #ef2027 !important;
        ...
        @font-face {
            font-family: 'Lato-Semibold';
            src: {{ asset('font/Lato-Semibold.ttf') }};
        }
    ...
    <div class="container-fluid" style="padding-left:0!important">
        <div class="row">
            <div class="col-lg-3 col-md-4 col-8">
                <h3 class="top-title">Update Meeting</h3>
            </div>
        </div>
    </div>
        ...
        <form id="maindata">
            <div class="container">
                ...
                <div class="row mt-4 mb-4" style="align-items: center;">
                    ...
                    <div class="col-md-8">
                        <input type="text" placeholder="Enter Meeting Name" name="title" id="title"
                            value="{{ $meeting->title }}">
                    </div>
                </div>
                ...
                <div class="row mt-4 mb-4" style="align-items: center;">
                    <div class="col-md-3">
                        <label class="title-input" for="datetimepicker3">Date And Time</label>
                    </div>
                    <div class="col-md-8">
                        <input type="text" placeholder="Enter Date And Time" name="datetime" id="datetimepicker3"
                ...
                <div class="row mt-4">
                    ...
                    <div class="col-md-12">
                        <textarea class="form-control" name="description" id="description" cols="30" rows="5">{{ $meeting->description }}</textarea>
                    </div>
                </div>

                <div class="row mt-4 mb-4" style="align-items: center;">
                    <div class="col-md-2">
                        <label class="title-input" for="agents">Agents</label>
                    </div>
                    <div class="col-md-12">
                        <select name="agents[]" id="agents" multiple>
                            @foreach ($meeting->users as $agent)
                ...
                <div class="row mt-5">
                    <div class="col-md-12 text-center">
                        <button id="buttonsubmitupdate" class="btn btn-primary" type="button">Update
                            <span id="spinner-border-update" class="spinner-border spinner-border-sm" role="status"
                                aria-hidden="true" hidden></span>
                        </button>
                        <button id="buttonsubmit" class="btn btn-primary" type="button">Attend
...
@push('scripts')
    ...
    <script>
        $('#buttonsubmit').click(function(e) {
            ...
            var url = "{{ route('attend.meeting', '#id') }}";
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.success) {
                        ...
                        window.location.href = "{{ route('list.meetings') }}";
                    } else {
                        $("#alertdata").empty();
        ...
        $('#buttonsubmitupdate').click(function(e) {
            e.preventDefault();
            var formData = new FormData($('#maindata')['0']);
            formData.append("_token", "{{ csrf_token() }}");
            formData.append("id", "{{ $meeting->id }}");
            $.ajax({
                method: 'post',
                processData: false,
                contentType: false,
                ...
                beforeSend: function() {
                    $('#buttonsubmitupdate').attr('disabled', 'disabled');
                    $('#spinner-border-update').removeAttr('hidden');
                },
                success: function(result) {
                    if (result.success) {
                        $("#alertdata").empty();
                        ...
                        $("#alertdata").attr('hidden', false);
                        $("#maindata")[0].reset();
                        $("#buttonsubmitupdate").removeAttr('disabled');
                        $('#spinner-border-update').attr('hidden', 'hidden');
                        window.location.href = "{{ route('list.meetings') }}";
                    } else {
                        $("#alertdata").empty();
                        $.each(result.message, function(key, value) {
