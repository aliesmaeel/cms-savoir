...
class ImportExcelController extends Controller
{
    ...
    {
        try {
            (new DataImport($filename))->import($pathToExcel);
        } catch (\Throwable $th) {
            dd($th->getMessage());
            //throw $th;
        }
        ...
        try {
            ...
            if ($Userimportingdata == 0 || $Userimportingdata == Auth::user()->id) {
                if ($Authuserimportstatus == 2 || $Authuserimportstatus == 1) {
                    ...
                    // get current data rows count
                    $currDataRowsCount = Data::count();
                    ...
                    // get new data rows count
                    $newDataRowsCount = Data::count();

                    uploadedFile::create([
                        'fileName' => $fileNameToStore,
                        'numberofimportedrows' => ($newDataRowsCount - $currDataRowsCount)
                    ]);

                    // send email
                    // Mail::to(Auth::user()->email)->send(new ImportingDataFinishedNotification(Auth::user()->getfilename()));
                    $details = ["file_name" => Auth::user()->getfilename()];
                    $this->fileImportedCompleted([["email" => Auth::user()->email]], $details);
    ...
    public function import_buildingView(Request $request){
        return view('import.import_building');
    }
  
    public function import_building(Request $request){
        Excel::import(new BuildingImport, 
